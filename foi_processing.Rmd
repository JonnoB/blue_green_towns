...
write_rds(ONSPD_df, file.path(data_folder, "lsoa_to_all_lookup.rds"))
```
...
colnames(scottish_data) <-c('lsoa11cd', 'all_properties', 'low_use', 'exempt')

scottish_data <- scottish_data  %>%
  mutate(type = "Scotland",
         low_use = low_use - exempt
       )%>% #exempt properties are excluded as this will likely include
  #soldier away on deployment, the recently deceeased etc
  select(-exempt) %>%
  mutate(low_use_ratio = low_use/all_properties,
         homes = all_properties - low_use) %>%
  left_join(., ONSPD_df %>%
